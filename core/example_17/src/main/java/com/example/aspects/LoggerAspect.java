...
public class LoggerAspect {
    ...
    //@AfterReturning(value = "@annotation(com.example.interfaces.LogAspect)", returning = "retVal")
    public void logStatus(JoinPoint joinPoint,Object retVal) {
        //System.out.println("VAO DAY ...AFTER");
        logger.log(Level.INFO,joinPoint.getSignature()+ "PH Method successfully processed with the status " +
                retVal.toString());
    }

    //@Before("@annotation(com.example.interfaces.LogAspect) && args(vehicleStarted,..)")
    ...
    public void checkVehicleStarted(JoinPoint joinPoint, boolean vehicleStarted) throws Throwable {
        //System.out.println("VAO DAY ...BEFORE");
        if(!vehicleStarted){
